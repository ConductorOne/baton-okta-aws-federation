...
func (o *accountResourceType) userGrants(ctx context.Context, resource *v2.Resource, attrs resource2.SyncOpAttrs, page string) ([]*v2.Grant, string, annotations.Annotations, error) {
	...
	if current.ResourceID != "" {
		...
		l.Debug("okta-aws-connector: processing queued user from bag", zap.String("user_id", userID))
	...
	l.Debug("okta-aws-connector: processing Okta user page",
		zap.Int("user_count", len(appUsers)),
		zap.String("current_page", oktaAfter),
		zap.String("next_page", nextOktaPage))
	...
	l.Debug("okta-aws-connector: no users need group role collection, advancing pagination",
		zap.String("next_page", nextOktaPage))
