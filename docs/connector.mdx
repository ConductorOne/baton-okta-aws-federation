...
description: "C1 provides identity governance and just-in-time provisioning for Okta for AWS federation. Integrate your Okta instance with C1 to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
og:description: "C1 provides identity governance and just-in-time provisioning for Okta for AWS federation. Integrate your Okta instance with C1 to run user access reviews (UARs), enable just-in-time access requests, and automatically provision and deprovision access."
...
**Provisioning scope.** This connector provisions AWS role access, not Okta identities. It assigns and revokes a SAML role on the AWS app — for a user directly assigned to the app, or for a group assigned to it — and it does not create or delete users or groups. It does not provision Okta group membership, and it does not manage Okta admin roles. Grant or revoke a user's membership in an Okta group through the paired Okta connector, not this one.
...
To give the C1 integration limited read-only admin permissions, you'll need to create a custom Okta admin role and resource set and assign them to the admin you'll use to generate the API key. 
...
**Optional.** If desired, click the checkbox to **Allow group to direct assignment conversion for provisioning**.
   ...
   When this setting is enabled, when AWS roles are requested directly and the user is assigned to the AWS app in Okta via group membership, C1 will convert the user assignment to a direct assignment.
</Step>
<Step>
Click **Save**.
</Step>
...
**That's it!** Your Okta AWS Federation connector is now pulling access data into C1.
</Tab>
